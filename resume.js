...
let radiusLength = canvas.width * canvas.height / 2000;
...
window.addEventListener("resize", function (event) {
    ...
    radiusLength = canvas.width * canvas.height / 2000;
    createParticles();
...
class Particle {
    ...
    draw() {
        ...
        ctx.arc(this.x, this.y, this.size, 0, Math.PI * 0.5, false);
...
function createParticles() {
    particlesArray = [];
    let noOfParticles = canvas.width * canvas.height / 3000;

    for (let i = 0; i < noOfParticles; i++) {
        let size = (Math.random() * 5) + 1;

        let x = (Math.random() * (innerWidth - 1 * size) + 1 * size);
        let y = (Math.random() * (innerHeight - 1 * size) + 1 * size);
...
function connect() {
    ...
    let vicinityDist = canvas.width * canvas.height / 40;
